...
//space ship by flaticon: https://www.flaticon.com/free-icon/space-invaders_706026#
...
int main()
{
	...
	static int screenHight = 1080;
	
	char space_ship_image_path[] = "space-invaders.png";
...
#pragma region Initialize
	...
	SDL_SetRenderDrawColor(renderer, 3, 34, 48, 255);
 
	// Loading an image
	// Here the surface is the information about the image. It contains the color data, width, height and other info.
	SDL_Surface* surface = IMG_Load(space_ship_image_path);
	if (!surface)
	{
		printf("Unable to load an image %s. Error: %s", space_ship_image_path, IMG_GetError());
		return -1;
	}
...
#pragma endregion
	...
	float spaceShipOffsetX = tex_width / 2;
	float spaceShipOffsetY = tex_height / 2;
	...
	// Space ship start point
	float x = 960.0f, y = 980.0f;
...
#pragma region GameLoop
	while (!done)
	{
		...
		if (x < tempX && x < screenWidth - spaceShipOffsetX) //Right
		{
			x = x + 0.2f * deltaTime;
		}
		else if (x > tempX && x > spaceShipOffsetX) //Left
		{
			x = x - 0.2f * deltaTime;
		}
		if (y < tempY && y < screenHight - spaceShipOffsetY) //Down
		{
			y = y + 0.2f * deltaTime;
		}
		else if (y > tempY && y > spaceShipOffsetY) //Up
		{
			y = y - 0.2f * deltaTime;
		}
		...
		SDL_Rect rect;
		rect.x = round(x - tex_width / 2); // Counting from the image's center but that's up to you
		rect.y = round(y - tex_height / 2); // Counting from the image's center but that's up to you
		rect.w = tex_width;
		rect.h = tex_height;
